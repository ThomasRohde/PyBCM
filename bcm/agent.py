...
class ChatDialog(ttk.Toplevel):
    def __init__(self, parent):
        ...
        try:
            ...
            self.entry = ttk.Entry(
                self.input_frame, textvariable=self.message_var, font=("Segoe UI", 11)
            )
    ...
    def _on_canvas_configure(self, event):
        try:
            ...
            self.chat_canvas.itemconfig(
                self.chat_canvas.find_withtag("all")[0], width=event.width
            )
            self._update_scroll_region()
        except Exception as e:
    ...
    def _convert_markdown_to_html(
        self, markdown_text: str, is_user: bool = False
    ) -> str:
...
<div class="{css_class}">
    ...
    def _send_message(self):
        ...
        try:
            ...
            threading.Thread(
                target=self._handle_ai_response, args=(message,), daemon=True
            ).start()
        except Exception as e:
    ...
    async def _fetch_and_display_response(self, message: str):
        ...
        try:
            ...
            async with agent.run_stream(
                message, message_history=self.messages, deps=deps
            ) as result:
                import time
                ...
                try:
                    ...
                    self.after(
                        ...
                        self.display_message,
                        "Assistant",
                        f"Error in stream handling: {str(e)}",
                    )
                    return
