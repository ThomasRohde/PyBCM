...
def add_wrapped_text(g: ET.Element, text: str, x: float, y: float, height: float, width: float, 
                    root_font_size: int, level: int, has_children: bool = False):
...
def add_node_to_svg(svg: ET.Element, node: LayoutModel, settings: Settings, level: int = 0):
